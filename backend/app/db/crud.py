...
async def mark_inventory_surplus(db: AsyncSession, inventory_id: str, seller_id: str):
    result = await db.execute(select(Inventory).where(Inventory.inventory_id == inventory_id, Inventory.user_id == seller_id))
...
async def update_surplus_discount(db: AsyncSession, inventory_id: str, seller_id: str, discount: float):
    result = await db.execute(select(Inventory).where(Inventory.inventory_id == inventory_id, Inventory.user_id == seller_id, Inventory.is_surplus))
    inventory = result.scalar_one_or_none()
    if not inventory:
...
async def get_my_surplus_items(db: AsyncSession, seller_id: str):
    result = await db.execute(select(Inventory).where(Inventory.user_id == seller_id, Inventory.is_surplus))
    return result.scalars().all()
...
async def buy_surplus_item(db: AsyncSession, inventory_id: str, buyer_id: str):
    result = await db.execute(select(Inventory).where(Inventory.inventory_id == inventory_id, Inventory.is_surplus))
    inventory = result.scalar_one_or_none()
