...
#             detail="Surplus inventory batch not found",
#         )
...
@router.post(
    "/add", response_model=InventoryResponse, status_code=status.HTTP_201_CREATED
)
...
async def get_product_inventory(
    ...
    return [
        InventoryResponse.from_orm_with_discount(batch) for batch in inventory_batches
    ]
...
async def get_my_inventory(
    ...
    query = (
        select(Inventory).join(Product).where(Inventory.user_id == current_user.user_id)
    )
...
async def get_available_quantity(
    product_id: str, db: AsyncSession = Depends(get_db_session)
):
    """
...
async def get_product_pricing(
    ...
    purchase_type: str = Query(
        "solo_singletime", regex="^(solo_singletime|subscription|group)$"
    ),
    db: AsyncSession = Depends(get_db_session),
):
    """
    ...
    return {
        ...
        "pricing": {
            ...
            "savings_percentage": float((savings / original_total) * 100)
            if original_total > 0
            else 0,
