...
from app.db.models import (
    BargainRoom, BargainBid, BargainMessage, BaseUser, Buyer, Seller, Product,
    BargainRoomCreate, BargainBidCreate, BargainMessageCreate,
    BargainRoomResponse, BargainBidResponse, BargainMessageResponse,
    BargainRoomWithDetailsResponse, PublicBargainResponse
)
...
@router.post("/public/create", response_model=BargainRoomResponse, status_code=status.HTTP_201_CREATED)
async def create_public_bargain(
    ...
    product_result = await db.execute(select(Product).where(Product.product_id == bargain_data.product_id))
    ...
    # Set expiry time
    expires_at = datetime.utcnow() + timedelta(hours=bargain_data.expires_in_hours)
...
async def get_available_public_bargains(
    ...
    # Build query
    query = select(BargainRoom).join(Product).where(
        and_(
    ...
    for room in bargain_rooms:
        # Get product details
        product_result = await db.execute(select(Product).where(Product.product_id == room.product_id))
        ...
        seller_responses = await db.execute(
            select(BargainBid).where(
                and_(
                    BargainBid.room_id == room.room_id,
                    BargainBid.user_type == "seller"
                )
            )
        )
        total_responses = len(seller_responses.scalars().all())
        
        public_bargains.append(PublicBargainResponse(
...
@router.post("/public/{room_id}/respond", response_model=BargainBidResponse, status_code=status.HTTP_201_CREATED)
async def respond_to_public_bargain(
    ...
    if room.expires_at and room.expires_at < datetime.utcnow():
        raise HTTPException(
            ...
            detail="This bargain has expired"
        )
...
@router.post("/private/create", response_model=BargainRoomResponse, status_code=status.HTTP_201_CREATED)
async def create_private_bargain(
    ...
    seller_result = await db.execute(select(Seller).where(Seller.user_id == bargain_data.seller_id))
    seller = seller_result.scalar_one_or_none()
...
@router.post("/{room_id}/bid", response_model=BargainBidResponse, status_code=status.HTTP_201_CREATED)
...
async def accept_bargain(
    ...
    if room.room_type == "public" and room.buyer_id == current_user.user_id:
        can_accept = True
    elif room.room_type == "private" and current_user.user_id in [room.buyer_id, room.seller_id]:
        can_accept = True
    ...
    bid_result = await db.execute(
        select(BargainBid).where(
            and_(
                BargainBid.bid_id == bid_id,
                BargainBid.room_id == room_id
            )
        )
    )
...
async def get_bargain_room_details(
    ...
    bids_result = await db.execute(
        select(BargainBid).where(BargainBid.room_id == room_id)
        .order_by(desc(BargainBid.created_at)).limit(10)
    )
    recent_bids = bids_result.scalars().all()
    
    # Get recent messages
    messages_result = await db.execute(
        select(BargainMessage).where(BargainMessage.room_id == room_id)
        .order_by(desc(BargainMessage.created_at)).limit(10)
    )
    ...
    return BargainRoomWithDetailsResponse(
        ...
        recent_messages=[BargainMessageResponse.model_validate(msg) for msg in recent_messages]
    )
...
async def get_user_from_websocket_token(websocket: WebSocket, db: AsyncSession) -> Optional[BaseUser]:
    """
...
async def verify_room_access(user: BaseUser, room_id: str, db: AsyncSession) -> bool:
    ...
    if room.buyer_id == user.user_id or room.seller_id == user.user_id:
        ...
        seller_result = await db.execute(select(Seller).where(Seller.user_id == user.user_id))
    ...
    try:
        ...
        while True:
            ...
            if message_type == "ping":
                ...
                messages_result = await db.execute(
                    select(BargainMessage).where(BargainMessage.room_id == room_id)
                    .order_by(desc(BargainMessage.created_at)).limit(20)
                )
                recent_messages = messages_result.scalars().all()
