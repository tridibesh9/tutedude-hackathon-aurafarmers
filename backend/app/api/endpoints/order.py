...
from app.db.models import (
    Order, OrderItem, BaseUser, Buyer, Seller, Product, Inventory, GroupOrderParticipant,
    OrderCreate, OrderItemCreate, OrderResponse, OrderWithItemsResponse,
    OrderItemResponse, DiscountStructure, GroupOrderJoinRequest, 
    GroupOrderParticipantResponse, GroupOrderSummary
)
...
async def create_order(
    ...
    order = Order(
        ...
        group_buyer_ids=[str(current_user.user_id)] if order_data.order_type == "group" else None,
...
async def calculate_order_pricing(
    ...
    for item in order_items:
        ...
        item_breakdowns.append({
            ...
            "savings_percentage": float((item_savings / item_original_price) * 100) if item_original_price > 0 else 0,
    ...
    return {
        ...
        "summary": {
            ...
            "overall_savings_percentage": float((total_savings / total_original_price) * 100) if total_original_price > 0 else 0
        },
...
async def join_group_order(
    ...
    # Get the order
    order_result = await db.execute(select(Order).where(Order.order_id == join_request.order_id))
    ...
    if order.order_type != "group":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="This is not a group order"
        )
...
async def get_group_order_details(
    ...
    if not (order.buyer_id == current_user.user_id or 
            order.seller_id == current_user.user_id or 
            is_participant):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    participants_result = await db.execute(
        select(GroupOrderParticipant, Buyer, BaseUser).join(
            Buyer, GroupOrderParticipant.buyer_id == Buyer.user_id
        ).join(
            BaseUser, Buyer.user_id == BaseUser.user_id
...
async def update_participant_status(
    ...
    participant_result = await db.execute(
        select(GroupOrderParticipant).where(GroupOrderParticipant.participant_id == participant_id)
    )
    ...
    order_result = await db.execute(select(Order).where(Order.order_id == participant.order_id))
    order = order_result.scalar_one_or_none()
    
    # Check permissions - only the participant themselves or the primary buyer can update status
    if not (participant.buyer_id == current_user.user_id or order.buyer_id == current_user.user_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def get_order_details(
    ...
    if order.group_buyer_ids:
        try:
            group_buyer_uuids = [uuid.UUID(buyer_id) for buyer_id in order.group_buyer_ids]
        except ValueError:
            group_buyer_uuids = None
