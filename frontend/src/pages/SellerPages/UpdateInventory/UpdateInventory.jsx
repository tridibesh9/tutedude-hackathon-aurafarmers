...
const UpdateInventory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const now = new Date();
      const expiryHours = productData.expiryUnit === 'hours' 
      ...
      const url = isUpdate ? `api/inventory/${existingItem.id}` : 'api/inventory/create';
      ...
      if (response.ok) {
        const message = isUpdate ? 'Product updated successfully!' : 'Product added successfully!';
        alert(message);
  ...
  return (
    <div className="update-inventory">
      ...
      <form onSubmit={handleSubmit} className="inventory-form">
        ...
        <div className="form-group">
          <label htmlFor="discountSubscriptionUser">Subscription User Discount (%)</label>
          <input
            type="number"
        ...
        <div className="form-group">
          <label htmlFor="discountMergedOrder">Merged Order Discount (%)</label>
          <input
            type="number"
          ...
          <div className="expiry-container">
            ...
            {productData.expiryValue && productData.expiryUnit && (
              <div className="expiry-preview">
                ...
                <span>
                  Expires on: {new Date(Date.now() + (
                    productData.expiryUnit === 'hours' 
                      ? parseInt(productData.expiryValue) 
                      : parseInt(productData.expiryValue) * 24
                  ) * 60 * 60 * 1000).toLocaleString('en-US', {
                    weekday: 'short',
                    year: 'numeric',
                    month: 'short',
                    day: 'numeric',
        ...
        <button 
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Saving...' : (isUpdate ? 'Update Product' : 'Add Product')}
        </button>
      </form>
    </div>
  );
