...
import { MapPin, Clock, CheckCircle, Truck, Phone, MessageCircle, RefreshCw } from 'lucide-react';
...
export const OrderTracking = ({ userRole }) => {
  ...
  const loadOrders = async () => {
    try {
      ...
      for (const status of orderStatus) {
        const response = await orderAPI.getAllOrders({ order_status: status, limit: 50 });
  ...
  const getStatusInfo = (status) => {
    switch (status?.toLowerCase()) {
      case 'pending':
        return { text: 'Order Placed', className: 'status-pending', color: '#f59e0b' };
      case 'confirmed':
        return { text: 'Confirmed', className: 'status-confirmed', color: '#3b82f6' };
      case 'shipped':
        return { text: 'Out for Delivery', className: 'status-shipped', color: '#8b5cf6' };
      case 'delivered':
        return { text: 'Delivered', className: 'status-delivered', color: '#10b981' };
      case 'cancelled':
        return { text: 'Cancelled', className: 'status-cancelled', color: '#ef4444' };
      default:
        return { text: 'Unknown', className: 'status-unknown', color: '#6b7280' };
    }
  };
      ...
      <div className="content-area">
        ...
        {!loading && orders.length === 0 && !error && (
          <div style={{
            ...
            <p>No {activeTab === 'live' ? 'active' : 'completed'} orders found</p>
        ...
        {!loading && orders.length > 0 && (
          <div className="orders-list">
            {orders.map((order) => {
              ...
              const availableStatuses = getAvailableStatusUpdates(order.order_status, userRole);
                ...
                <div key={order.order_id} className="live-order-card">
                  ...
                  <div className="live-order-header">
                    <div className="flex-between">
                      <div>
                        <div className="order-id-label">Order #{order.order_id.slice(0, 8)}</div>
                        <div className="supplier-name-lg">
                          {userRole === 'seller' ? `Buyer: ${order.buyer_id.slice(0, 8)}...` : `Seller: ${order.seller_id.slice(0, 8)}...`}
                        </div>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        {order.estimated_delivery_date && (
                          <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>
                            ETA: {apiHelpers.formatDate(order.estimated_delivery_date)}
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div style={{ padding: '12px 0' }}>
                    ...
                    {order.order_items && order.order_items.length > 0 ? (
                      <div style={{ display: 'grid', gap: '4px' }}>
                        {order.order_items.slice(0, 3).map((item, index) => (
                          <div key={index} style={{ 
                            ...
                            <span>{item.quantity} × ₹{item.price_per_unit}</span>
                          </div>
                        ))}
                      ...
                      <div style={{ fontSize: '14px', color: '#6b7280' }}>No items found</div>
                    )}
                  </div>
